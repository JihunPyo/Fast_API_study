...
from schema.response import TodoResponse #응답 모델 불러오기
from schema.request import TodoCreateRequest, TodoUpdateRequest #생성 요청 모델 불러오기
...
@app.get(
    "/todos",
    response_model=list[TodoResponse], #TOdoResponse 객체로 이루어진 리스트 반환
...
def create_todo_handler(body: TodoCreateRequest): #요청 본문을 body 매개변수로 받고 타입을 TodoCreateRequest로 지정
    new_todo = { #새 할일 데이터 생성하고 new_todo에 저장
        "id": len(todos) +1,
        "title": body.title,
        "is_done": body.is_done,
    }
    todos.append(new_todo) #new_todo를 todos 리스트에 추가
